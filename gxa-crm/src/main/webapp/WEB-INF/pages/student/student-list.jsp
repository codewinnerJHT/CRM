    ...
    <div class="x-body">
      ...
      <div>
        <div class="layui-form layui-col-md12 x-so">
          ...
          <button class="layui-btn" id="search" onclick="getData(1,10)"><i class="layui-icon">&#xe615;</i></button>
        </div>
    ...
    <script>
      layui.use(['laydate','upload'], function(){
        ...
        var uploadInst = upload.render({
          elem: '#file' //绑定元素
          ,url: '${pageContext.request.contextPath}/student/do/upload' //上传接口
          ,accept:'file'
          ,exts:'xls|xlsx'
          ,field:'file'
          ,done: function(res){
      ...
      function getData(pageNum,pageSize){
        ...
        $.ajax({
          type:"get",
          url:"${pageContext.request.contextPath}/student/list/all/data",
          data:param,
          dataType:"json",
          success:function (data) {
            console.log(data);
            if (data.code === 200){
              ...
              $(data.data.list).each(function () {
                var tr = $('          <tr>\n' +
                        ...
                        '            <td>'+this.stuName+'</td>\n' +
                        '            <td>'+this.stuAge+'</td>\n' +
                        '            <td>'+arr1[this.stuGender]+'</td>\n' +
                        '            <td>'+this.stuCourseIntention+'</td>\n' +
                        '            <td>'+this.parentPhone+'</td>\n' +
                        '            <td>'+this.activityId+'</td>\n' +

                        '            <td>'+arr2[this.status]+'</td>\n' +
